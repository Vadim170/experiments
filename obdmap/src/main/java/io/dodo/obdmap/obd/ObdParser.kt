...
object ObdParser {
    ...
    /**
     * Выделяет ответ на последнюю команду из возможно склеенных.
     *
     * ELM327 завершает каждый ответ приглашением '>'. Если предыдущая команда
     * ответила с опозданием (клон задумался, связь моргнула), её хвост приходит
     * уже после того, как мы отправили следующую, и в буфере оказывается
     * несколько приглашений сразу: "OK\r>ELM327 v2.1\r>". Наш ответ —
     * последний: всё, что пришло раньше, относится к предыдущей команде.
     *
     * Без этого ответы разъезжаются на одну команду, и следующая ждёт свой
     * ответ, который уже съеден, до самого таймаута.
     */
    fun lastResponse(raw: String): String {
        val segments = raw.split('>')
            .map { it.trim(' ', '\r', '\n', '\t') }
            .filter { it.isNotEmpty() }
        return segments.lastOrNull() ?: raw
    }
